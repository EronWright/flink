...
public class LaunchableMesosWorker implements LaunchableTask {
	...
	public Protos.TaskInfo launch(Protos.SlaveID slaveId, MesosResourceAllocation allocation) {
		...
		// take needed resources from the overall allocation, with the assumption that adequate resources exist
		Set<String> roles = Collections.singleton(mesosConfiguration.frameworkInfo().getRole());

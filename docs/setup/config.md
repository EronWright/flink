...
- `akka.ssl.enabled`: Turns on SSL for Akka's remote communication (DEFAULT: **false**).
- `akka.ssl.keystore`: The Java keystore file to be used by the Akka endpoint for its SSL Certificate.
...
- `akka.ssl.truststore`: The truststore file containing the public CA certificates to be used by akka endpoints to verify the peer's certificate.
...
- `akka.ssl.random-number-generator`: The random number generator to be used depending on the security requirements. For available values see [here](http://doc.akka.io/docs/akka/snapshot/general/configuration.html#akka-remote) (DEFAULT: **SecureRandom**).
